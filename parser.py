...
def round_number(value: Any, decimals: int = NUMERIC_DECIMALS) -> float | None:
...
def extract_dimension_specs(
    ...
    for descriptor in dimension_columns:
        ...
        nominal_value = round_number(nominal_value)
        positive_tolerance = round_number(positive_tolerance)
        negative_tolerance = round_number(negative_tolerance)
        listed_upper = round_number(listed_upper)
        listed_lower = round_number(listed_lower)
        calculated_upper = round_number(nominal_value + positive_tolerance) if positive_tolerance is not None else None
        calculated_lower = round_number(nominal_value - negative_tolerance) if negative_tolerance is not None else None
        upper_match = (
    ...
    for row_idx in range(equipment_row + 1, ws.max_row + 1):
        ...
        for spec in dimension_specs.values():
            ...
            if measurement_value is None:
                continue
            measurement_value = round(measurement_value, NUMERIC_DECIMALS)
...
def validate_measurement(
    ...
    if measurement is None or lower_limit is None or upper_limit is None:
        return "SPEC_MISSING"
    return "PASS" if (lower_limit - COMPARISON_EPSILON) <= measurement <= (upper_limit + COMPARISON_EPSILON) else "FAIL"
        ...
        try:
            ...
            for record in sample_records:
                ...
                difference_from_nominal = (
                    round(record["measurement_value"] - spec.nominal_value, NUMERIC_DECIMALS)
